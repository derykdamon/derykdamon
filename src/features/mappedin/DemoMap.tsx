...
function hidePlaceholderFloorButtons(): (() => void) | undefined {
  ...
  const controls = heading?.closest('div.flex')?.lastElementChild as HTMLElement | null
...
function setDefinitionValue(term: string, value: string) {
  const target = Array.from(document.querySelectorAll('dt')).find(
...
function updateDashboardLocation(
  ...
  if (details) details.textContent = `${floorName} · Live Mappedin space`
...
function DemoMapContent() {
  ...
  const loadMap = useCallback(async () => {
    ...
    mapView.on('click', (event) => {
      ...
      if (label) setSelectedSpace(label.text)
    })
  ...
  return (
    <div ref={panelRef} className="absolute inset-0 z-20 overflow-hidden bg-[#070b16]">
      ...
      {loadState === 'loading' && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#070b16]">
          <div className="rounded-2xl border border-white/10 bg-[#0b1120]/95 px-8 py-7 text-center shadow-2xl backdrop-blur-xl">
            <LoaderCircle size={30} className="mx-auto animate-spin text-cyan-300" />
            <p className="mt-4 text-sm font-medium text-white">Loading Robley Rex VAMC</p>
            <p className="mt-1 text-xs text-slate-500">Securing map access and preparing the digital twin</p>
          </div>
        </div>
      )}
      ...
      {loadState === 'error' && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#070b16] px-6">
          <div className="max-w-md rounded-2xl border border-red-300/20 bg-red-300/[0.06] p-7 text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-white">Unable to load the facility map</h2>
            <p className="mt-2 text-sm leading-6 text-slate-400">{errorMessage}</p>
            <button type="button" onClick={() => setReloadKey((value) => value + 1)} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10">
      ...
      {loadState === 'ready' && (
        <>
          <div className="pointer-events-none absolute left-4 top-4 rounded-full border border-emerald-300/20 bg-[#07111a]/85 px-3 py-1.5 text-xs font-medium text-emerald-200 backdrop-blur-md">
            <span className="mr-2 inline-block h-2 w-2 rounded-full bg-emerald-300" /> Live digital twin
          </div>
          ...
          <div className="absolute right-4 top-4 flex max-w-[calc(100%-2rem)] flex-col items-end gap-2">
            <div className="flex max-w-full flex-wrap justify-end gap-2 rounded-2xl border border-white/10 bg-[#07111a]/90 p-2 shadow-xl backdrop-blur-md">
              {floors.map((floor) => (
                <button key={floor.id} type="button" onClick={() => mapViewRef.current?.setFloor(floor.id)} className={`rounded-lg border px-3 py-2 text-xs font-medium transition ${currentFloorId === floor.id ? 'border-cyan-300/40 bg-cyan-300/15 text-cyan-100' : 'border-white/10 bg-white/[0.03] text-slate-300 hover:bg-white/10'}`}>
                  {floor.name}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <button type="button" onClick={() => rotateCamera(-90)} className="rounded-xl border border-white/10 bg-[#07111a]/90 p-2.5 text-slate-200 hover:bg-white/10" title="Rotate left"><RotateCcw size={16} /></button>
              <button type="button" onClick={() => rotateCamera(90)} className="rounded-xl border border-white/10 bg-[#07111a]/90 p-2.5 text-slate-200 hover:bg-white/10" title="Rotate right"><RotateCw size={16} /></button>
              <button type="button" onClick={toggleCameraMode} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-[#07111a]/90 px-3 py-2 text-xs text-slate-200 hover:bg-white/10"><Compass size={15} />{cameraMode === '3d' ? 'Top view' : '3D view'}</button>
              <button type="button" onClick={resetCamera} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-[#07111a]/90 px-3 py-2 text-xs text-slate-200 hover:bg-white/10"><RotateCcw size={15} />Reset</button>
              <button type="button" onClick={() => void toggleLabels()} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-[#07111a]/90 px-3 py-2 text-xs text-slate-200 hover:bg-white/10"><Tags size={15} />{labelsVisible ? 'Hide labels' : 'Show labels'}</button>
              <button type="button" onClick={() => void enterFullscreen()} className="rounded-xl border border-white/10 bg-[#07111a]/90 p-2.5 text-slate-200 hover:bg-white/10"><Expand size={16} /></button>
            </div>
          </div>

          <div className="pointer-events-none absolute left-4 top-14 inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#07111a]/80 px-3 py-1.5 text-[11px] text-slate-300"><Layers3 size={13} />{floors.length} mapped floors loaded</div>
          <div className="pointer-events-none absolute left-4 top-24 hidden max-w-xs items-center gap-2 rounded-xl border border-white/10 bg-[#07111a]/80 px-3 py-2 text-[11px] leading-4 text-slate-300 md:flex"><MousePointer2 size={14} className="shrink-0 text-cyan-300" />Drag to pan. Hold Control while dragging, or use the rotate buttons, to orbit a full 360°.</div>

          <div className="absolute bottom-4 left-4 max-w-sm rounded-2xl border border-white/10 bg-[#07111a]/90 p-3 text-xs text-slate-300 shadow-xl">
            <div className="flex items-center gap-2 font-medium text-white"><MapPin size={15} className="text-cyan-300" />{selectedSpace}</div>
            <div className="mt-2 flex items-center gap-2 text-[11px] text-slate-400"><Search size={13} />Use the top search box and press Enter.</div>
          </div>

          <div className="absolute bottom-4 right-4 w-[min(24rem,calc(100%-2rem))] rounded-2xl border border-white/10 bg-[#07111a]/92 p-3 shadow-xl">
            <button type="button" onClick={requestLocation} disabled={locationState.status === 'requesting'} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-cyan-200 disabled:opacity-60">
              {locationState.status === 'requesting' ? <LoaderCircle size={15} className="animate-spin" /> : <LocateFixed size={15} />} Use my location
            </button>
...
function DemoMap() {
  ...
  useEffect(() => {
    ...
    if (locateTarget()) return () => restoreFloorControls?.()
